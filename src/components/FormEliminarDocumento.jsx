...
import { addDocumentoEliminado } from '@services/api/documentosEliminados';


export default function FormEliminarDocumento({ documento, handleDelete, setOpenDeleteModal, setOpen }) {
  ...
  const handleSubmit = async (event) => {
    ...
    const data = {
      ...
      año: documento.archivo.año,
      oficio: formData.get('numeroOficioCarta'),
      acta: formData.get('numeroActaSesiónComiteEvaluador'),
    };
    if (data.oficio === '' || data.folio === null) {
      alert('El campo de numero de oficio de la carta es obligatorio');
    }
    if (data.acta === '' || data.acta === null) {
      alert('El campo de numero de acta de la sesión de comite de evaluador es obligatorio');
    }
    addDocumentoEliminado(data)
      .then(() => {
      ...
      <div>
        <div className="w-full">
          <div className="px-1 py-1 sm:px-2">
            <h5 className="text-sm leading-6 font-medium text-gray-900">{documento.titulo}</h5>
          </div>
        </div>
