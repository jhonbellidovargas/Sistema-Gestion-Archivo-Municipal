...
export default function FormArchivo({ setOpen, setAlert, prestamo }) {
  ...
  const buscarArchivo = () => {
    ...
    const archivo = archivos.find((archivo) => archivo.id === id);
  ...
  const handleSubmit = async (event) => {
    ...
    if (prestamo) {
      ...
      if (buscarArchivo() === 'El archivo se encuentra prestado' || buscarTrabajador() === 'El DNI no está registrado') {
        alert(buscarArchivo() || buscarTrabajador());
      } else {
        addPrestamo(data)
          .then(() => {
            setAlert({
